...
public class FindRestaurantsActivity extends Activity {
	...
	public void searchButtonHandler(View V) {
		...
		if (!(keyword.trim().length() == 0)) {
			...
			if (!(rawzip.trim().length() == 0)) {
				...
				app.setSearchText(keyword + " near " + zip);
			}
			...
			if (restaurants != null) {
				...
				Toast.makeText(getApplicationContext(),
						"No Restaurants Found.", Toast.LENGTH_SHORT).show();
				// searchButton.setText("Search");
			}
		} else {
	...
	protected void makeUseOfNewLocation(Location location) {
		...
		if (restaurants != null) {
			...
			Toast.makeText(getApplicationContext(), "No Restaurants Found",
					Toast.LENGTH_SHORT).show();
	...
	//
	// for (i = 0; i < 10; i++) {
	// a = new Restaurant("Restaurant " + i, i*2 + " Main St", "407-555-1212",
	// "GPS Coords");
	// restaurants.add(a);
	// }
	//
